...
2. 系统会自动检测并显示文件夹中所有可用的文件后缀名
3. 选择要修改的文件类型（支持多种后缀名，如jpg, png, gif → webp）
4. 点击后缀名按钮选择要修改的文件类型，或手动输入后缀名
5. 输入新的后缀名（不需要包含点号）
6. 点击"更新文件列表"查看将要修改的文件
7. 点击"预览文件更改"确认更改内容
8. 点击"下载重命名脚本"获取可执行的脚本文件（系统会自动选择适合您平台的脚本）
9. 将脚本放在目标文件夹中运行，执行实际的文件重命名
